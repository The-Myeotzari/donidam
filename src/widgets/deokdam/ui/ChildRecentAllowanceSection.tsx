'use client'

import { useChildAllowancesQuery } from '@/entities/deokdam/api/deokdam.child.queries'
import { ChildAllowanceCard } from '@/widgets/deokdam/ui/ChildAllowanceCard'
import { ROUTES } from '@/shared/constants/route'
import { ChevronRight } from 'lucide-react'
import Link from 'next/link'

export function ChildRecentAllowanceSection() {
  const { data } = useChildAllowancesQuery()
  const items = (data?.items ?? []).slice(0, 2)

  return (
    <div className="mt-5">
      <div className="flex items-center justify-between mb-2">
        <h3 className="text-md font-semibold">최근 덕담</h3>
        <Link
          href={ROUTES.deokdamChildRecent}
          className="flex items-center gap-0.5 text-xs text-muted-foreground"
        >
          전체보기 <ChevronRight size={14} />
        </Link>
      </div>

      {items.length === 0 ? (
        <div className="rounded-2xl bg-card p-5 shadow-sm text-center">
          <p className="text-sm text-muted-foreground">받은 덕담이 없어요</p>
        </div>
      ) : (
        <div className="space-y-2">
          {items.map((item) => (
            <ChildAllowanceCard key={item.id} item={item} />
          ))}
        </div>
      )}
    </div>
  )
}
